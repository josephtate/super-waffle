...
def test_select_mirror_returns_non_empty_urls(provider, region):
    """Test select_mirror always returns two non-empty URLs."""
    yaml_path = FIXTURES_DIR / "mock-mirrors.yaml"
    mirror_map = load_mirror_map(str(yaml_path))
    
    primary, backup = select_mirror({"provider": provider, "region": region}, mirror_map)
...
def test_select_mirror_provider_fallback():
    """Test select_mirror falls back to provider default."""
    yaml_path = FIXTURES_DIR / "mock-mirrors.yaml"
    mirror_map = load_mirror_map(str(yaml_path))
    
    primary, backup = select_mirror({"provider": "azure", "region": "unknown"}, mirror_map)
    assert primary == "https://depot.eastus.prod.azure.ciq.com"

def test_select_mirror_global_fallback():
    """Test select_mirror falls back to global default."""
    yaml_path = FIXTURES_DIR / "mock-mirrors.yaml"
    mirror_map = load_mirror_map(str(yaml_path))
    
    primary, backup = select_mirror({"provider": "unknown", "region": "unknown"}, mirror_map)
